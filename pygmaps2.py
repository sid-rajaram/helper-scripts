...
class maps:
	...
	def addsquare(self, lat,lng,side,color = '#0000FF',opacity=1.0,strokeWeight=1):
		self.squares.append((lat,lng,side,color,opacity,strokeWeight))
	...
	def drawsquares(self, f):
		for sq in self.squares:
			...
			opacity = sq[4]
			strokeWeight = sq[5]
			path = [(lat,lng),(lat+side,lng),(lat+side,lng+side),(lat,lng+side)]
			self.drawPolygon(f,path,fillColor=color,fillOpacity=opacity,strokeColor=color,strokeOpacity=opacity,strokeWeight=strokeWeight)
